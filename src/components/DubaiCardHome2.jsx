...
const DubaiCardHome2 = () => {
  const { symbol, rate } = useCurrency();
  const navigate = useNavigate();
  return (
    <div className="bg-white rounded-xl shadow-lg p-8 flex flex-col items-center max-w-md w-full">
      <ImageCarouselHome2 images={[tourImg01, tourImg02, tourImg03]} alt="Dubai" className="w-full h-64 object-cover rounded-lg mb-4" />
      ...
      <ButtonHome2 onClick={() => navigate("/dubai")}>Explore</ButtonHome2>
    </div>
  );
};

export default DubaiCardHome2; 